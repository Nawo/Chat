...
namespace chat
{
	namespace net
	{
		// Message Header is sent at start of all messages. The template allows
		// us to use "enum class" to ensure that the messages are valid at
		// compile time
		template <typename T>
		...
		// Message Body contains a header and a std::vector, containing raw
		// bytes of infomation. This way the message can be variable length, but
		// the size in the header must be updated.
		template <typename T>
		struct message
		{
			// Header & Body vector
